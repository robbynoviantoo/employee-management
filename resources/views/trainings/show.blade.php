...
@section('content')
    <div class="container mt-4">
        ...
        <div class="row mb-4">
            <div class="col-md-3">
                @if($employee->photo)
                    <img src="{{ asset('storage/app/public/' . $employee->photo) }}" alt="Foto {{ $employee->nik }}" class="img-fluid img-thumbnail custom-img">
                @else
                    <img src="{{ asset('images/default-avatar.png') }}" alt="Foto {{ $employee->nik }}" class="img-fluid img-thumbnail custom-img">
                @endif
            </div>
            <div class="col-md-9">
                <h4>{{ $employee->name }}</h4>
                ...
                <div class="dropdown mt-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                        @foreach($categories as $category)
                            <li><a class="dropdown-item" href="#" onclick="showTable('{{ $category }}')">{{ $category }}</a></li>
                        @endforeach
                    </ul>
                </div>
            </div>
